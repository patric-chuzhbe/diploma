...
func TestIndexRoute(t *testing.T) {
	t.Run("test index endpoint", func(t *testing.T) {
...
func TestRegisterLoginCycle(t *testing.T) {
	t.Run("test register login cycle", func(t *testing.T) {
		err := logger.Init(logLevel)
		require.NoError(t, err)
...
func TestPostApiUserRegisterValidationError(t *testing.T) {
	t.Run("test POST /api/user/register validation error", func(t *testing.T) {
		err := logger.Init(logLevel)
		require.NoError(t, err)
		...
		require.Equal(t, http.StatusBadRequest /*StatusUnauthorized*/, resp.Code)
	})
}

func TestPostApiUserRegisterConflict(t *testing.T) {
	t.Run("test POST /api/user/register conflict", func(t *testing.T) {
...
func TestPostApiUserLoginEmptyCredentials(t *testing.T) {
	t.Run("test POST /api/user/login empty credentials", func(t *testing.T) {
		db := new(mockstorage.MockStorage)
		mockAuth := new(mockauthenticator.MockAuthenticator)
		r := New(db, mockAuth)
...
func TestPostApiUserOrders(t *testing.T) {
	...
	//a := auth.New(db, authCookieName, []byte(authCookieSigningSecretKey))
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Equal(t, tt.expectedStatus /*StatusUnauthorized*/, resp.Code)

			//------------------------------------------------------
			//mockUC := new(mockOrderUsecase)
			//
			//// Only validate and call CreateOrder if valid number
			//if len(tt.orderNumber) >= 10 && isDigits(tt.orderNumber) {
			//	mockUC.On("CreateOrder", tt.userID, tt.orderNumber).Return(tt.mockErr)
			//}
			//
			//handler := router.NewOrderHandler(mockUC)
			//req := httptest.NewRequest(http.MethodPost, "/api/user/orders", bytes.NewBufferString(tt.orderNumber))
			//rec := httptest.NewRecorder()
			//
			//// Inject user ID
			//handlerWithAuth := mockAuthMiddleware(tt.userID)(http.HandlerFunc(handler.PostOrder))
			//
			//handlerWithAuth.ServeHTTP(rec, req)
			//
...
func TestPostApiUserOrdersBadContentType(t *testing.T) {
	t.Run("test POST /api/user/orders bad content type", func(t *testing.T) {
		err := logger.Init(logLevel)
		require.NoError(t, err)
...
func TestGetApiUserBalanceInternalServerError(t *testing.T) {
	t.Run("test GET /api/user/balance internal server error", func(t *testing.T) {
		db := new(mockstorage.MockStorage)
		mockAuth := new(mockauthenticator.MockAuthenticator)
		r := New(db, mockAuth)
		...
		db.On(
			...
			Return(
				/*nil*/ &models.UserBalanceAndWithdrawals{},
				errors.New("db error"),
			)
...
func TestWithdrawSuccess(t *testing.T) {
	...
	//if false {
	//	logFile, _ := os.OpenFile("log00for_diploma.internal.router.router_test.go.TestWithdrawSuccess.log", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	//	_, _, line, _ := runtime.Caller(0)
	//	fmt.Fprintln(logFile, "line: "+strconv.Itoa(line)+"\n\tuser: "+dumpAnyStructByGubanov(user)+"\n\n")
	//	logFile.Close()
	//}
	/*</testCode>*/

	//db := new(mockstorage.MockStorage)
	//authMock := new(mockauthenticator.MockAuthenticator)
	//handler := handlers.NewBalanceHandler(db, authMock)
	//
	//userID := "test-user-id"
	//order := "2377225624"
	//withdrawSum := float32(100.0)
	...
	r.ServeHTTP(rec, req)
	//handler.PostApiuserbalancewithdraw(rec, req)
...
func TestWithdrawValidationFail(t *testing.T) {
	...
	reqBody := `{"order": "5857088487"}` /*, "sum": 0*/
...
func TestWithdrawAlreadyWithdrawn(t *testing.T) {
	...
	reqBody2, _ := json.Marshal(models.BalanceWithdrawRequest{
		OrderNumber:/*"2503317444",*/ "3376308833",
		WithdrawSum: 30,
	})
	...
	assert.Equal(t /*http.StatusOK*/, http.StatusUnprocessableEntity, rec2.Code)
}
